...
// @access  Teacher
const bulkReport = async (req, res) => {
  const { examName, examDate, maxMarks, reports } = req.body;
  // reports: [{ studentId, subject, marks, grade, remarks }]
  try {
    ...
    res.status(201).json({ message: 'Reports uploaded', reports: createdReports });
  } catch (error) {
    console.error(error);
...
const getStudents = async (req, res) => {
  try {
    ...
    // Match students by className and section using new classesHandled structure
    const students = await Student.find({
      $or: teacher.classesHandled.map(cls => ({
